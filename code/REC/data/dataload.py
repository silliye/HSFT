...
class Data:
    ...
    def _from_scratch(self):
        ...
        # --- [代码修改] ---
        # 在数据处理完成后，检查是否需要加载缓存
        self._load_item_embedding_cache()
        # --------------------

    # --- [代码修改] ---
    # 新增一个方法，专门用于加载Item Embedding缓存文件
    def _load_item_embedding_cache(self):
        """
        如果配置中启用了use_item_cache，则加载预计算的物品Embedding。
        """
        if self.config['use_item_cache']:
            # config['root_path'] 通常是项目根目录，我们需要拼接出缓存文件的绝对路径
            # config['item_cache_path'] 是我们在main.py中添加的新参数
            hllm_root_path = './'
            cache_path = os.path.join(hllm_root_path, self.config['item_cache_path'])
            self.logger.info(f"正在从 '{cache_path}' 加载Item Embedding缓存...")
            
            if not os.path.exists(cache_path):
                raise FileNotFoundError(f"缓存文件未找到: {cache_path}. 请先运行cache_items.py脚本生成缓存。")
            
            # 使用torch.load加载缓存文件，并将其存储在类的属性中
            self.item_embeddings_cache = torch.load(cache_path, map_location='cpu')
            self.logger.info(f"缓存加载成功！共加载了 {len(self.item_embeddings_cache)} 个物品的Embeddings。")
    # --------------------
