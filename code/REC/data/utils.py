...
def bulid_dataloader(config, dataload):
    ...
    dataset_dict = {
        ...
        'HLLM': (('CachedEmbeddingTrainDataset', 'cached_train_collate_fn'), 'CachedEmbeddingEvalDataset', 'cached_eval_collate_fn')
    }
    ...
    if dist.is_initialized():
        # 分布式模式：使用分布式采样器
        # 假设 NonConsecutiveSequentialDistributedSampler 也是一个分布式采样器
        ...
        rank = dist.get_rank()
        # 在分布式模式下，每个worker使用不同的seed
        seed = torch.initial_seed()
        init_fn = partial(
            worker_init_fn, num_workers=config.get('num_workers', 1), rank=rank,
            seed=seed
        )
    else:
        # 单卡模式：不使用采样器 (设为 None)，DataLoader会使用默认行为
        train_sampler = None
        valid_sampler = None
        test_sampler = None
        rank = 0 # rank 默认为 0
        # 在单卡模式下，所有worker可以使用相同的seed
        init_fn = None # 或者一个不依赖rank的简单init_fn
